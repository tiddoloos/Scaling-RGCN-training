...
def get_node_mappings_dict(path):
    ...
    return sorted(orgNode2sumNode_dict), sumNode2orgNode_dict

def vectorize_node_mapping(sumNode2orgNode_dict, sum2types_dict, org2type_dict, labels_dict, num_classes):
    sg2label_vec = defaultdict()
    g2label_vec = defaultdict()
    for sumNode, orgNodes in sumNode2orgNode_dict.items():
        ...
        for node in orgNodes:
            ...
            g2label_vec[node] = g_labels
        for i in range(len(sg_labels)):
            sg_labels[i] = sg_labels[i] / len(orgNodes)
        sg2label_vec[sumNode] = sg_labels
    return sg2label_vec, g2label_vec 

def main(dataset_name):
    dataset_name = 'AIFB'
    org_file = f'data/{dataset_name}/{dataset_name}_complete.n3'
    sum_file = f'data/{dataset_name}/{dataset_name}_attr_sum.n3'
    ...
    #difference in the amount of types/labels
    labels1, org2type_dict = nodes2type_mapping(org_file)
    labels2, sum2types_dict = nodes2type_mapping(sum_file)
    labels = labels2
    if len(labels1) > len(labels2):
        labels = labels1
    
    labels_dict = {lab: i for i, lab in enumerate(labels)}
    sg2label_vec, g2label_vec = vectorize_node_mapping(sumNode2orgNode_dict, sum2types_dict, org2type_dict, labels_dict, len(labels))
    #todo: return the right dicts etc
